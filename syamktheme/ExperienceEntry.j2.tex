...
\begin{twocolentry}{<<date_and_location_strings|join("\n\n")>>} 
    
    \textbf{  ((* if entry.company2 *))  {\color{primaryColor}<<entry.company2>>} $\rightarrow$ ((* endif *)){\color{primaryColor}<<entry.company>>}}, 
    <<entry.position>> ((* if entry.position2 *)) $\rightarrow$ <<entry.position2>> ((* endif *))
